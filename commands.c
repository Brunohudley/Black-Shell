...
win_proc (char **args)
{
  ...
  if (!CreateProcessA (NULL, wincmdl, NULL, NULL, FALSE, 0, NULL, NULL, &si,
                       &pi))
    {
      ...
      for (int count = 0; count <= maxarg; count++)
        {
          ...
          for (int line = 1;
               fgets (buffer, sizeof (buffer), file) && line <= 10; line++)
            {
              printf ("%s", buffer);
            }
          fclose (file);
...
int
lchmod (const char *filepath, const char *expr)
{
  // stat st and stat()
  struct stat st;
  ...
  while (*pexpr
         && (*pexpr == 'u' || *pexpr == 'g' || *pexpr == 'o' || *pexpr == 'a'))
    {
      if (*pexpr == 'u')
        userp = 1;
      if (*pexpr == 'g')
...
sh_uname (char **args)
{
  ...
  if (RegOpenKeyExA (HKEY_LOCAL_MACHINE, hardware_path, 0, KEY_READ, &key)
      != ERROR_SUCCESS)
    {
      return error_set ("Cannot Read CPU Name!\n");
    }
  if (RegQueryValueExA (key, "ProcessorNameString", NULL, NULL, (LPBYTE)name,
                        &ssize)
      == ERROR_SUCCESS)
    {
      printf ("CPU Name: %s\n", name);
...
funcNew ()
{
  if (shell.fun_props.func_cx >= shell.fun_props.func_capacity)
    {
      shell.fun_props.func_capacity
          = (shell.fun_props.func_capacity == 0)
                ? 4
                : shell.fun_props.func_capacity * 2;

      t_func *newfns = realloc (
          shell.func, sizeof (t_func) * shell.fun_props.func_capacity);
      if (!newfns)
        {
  ...
  return &shell.func[shell.fun_props.func_cx++];
}
...
func_addLine
            ...
            * see if the function line count is greater than it own capacity,
            ...
            in the end, it percur the array, and set that part as the argument
line
...
exec_fun (char *name)
{
  for (int i = 0; i < shell.fun_props.func_cx; i++)
    {
      if (strcmp (shell.func[i].name, name) == 0)
        {
          for (int j = 0; j < shell.func[i].line_count; j++)
            {
              char **cmd = sh_splitLine (shell.func[i].lines[j]);
              sh_execute (cmd);
              free (cmd);
            }
          return 1;
