...
get_var		arguments:
                        ...
                        details:
                        LET "-i"		| return a pointer of the
integer value of vars.data. LET "-f"		| return a pointer of the float
value of vars.data. LET "-s" 		| return a pointer of the string value
of vars.data.
...
void *
get_var (const char *name, const char *expr)
{
  for (int i = 0; i < shell.var_count; i++)
    {
      if (strcmp (shell.var[i].name, name) == 0)
        {
          if (strcmp (expr, "-i") == 0)
            {
...
set_var		arguments:
                        ...
                        details:
                        EXPR "-i"		| set vars.data integer value
into the type casted var of value. EXPR "-f"		| set vars.data float
value into the type casted var of value. EXPR "-s" 		| set vars.data
string 	value into the type casted var of value.
...
void
set_var (const char *expr, const char *name, void *value)
{
  for (int i = 0; i < shell.var_count; i++)
    {
      if (strcmp (shell.var[i].name, name) == 0)
  ...
  if (strcmp (expr, "-i") == 0)
  ...
  else if (strcmp (expr, "-s") == 0)
    {
      strncpy (shell.var[shell.var_count].data.str_value, (char *)value,
               sizeof (shell.var[shell.var_count].data.str_value));
    }
...
int
check_cmp_flag (int type)
{
  if (type == CMP_EQUAL)
    return shell.cmp_flag == CMP_EQUAL;

  if (type == CMP_GREATER)
    return shell.cmp_flag == CMP_GREATER;

  if (type == CMP_LESS)
    return shell.cmp_flag == CMP_LESS;

  if (type == CMP_NEQUAL)
    return shell.cmp_flag != CMP_EQUAL;

  if (type == CMP_GEQUAL)
    return (shell.cmp_flag == CMP_GREATER || shell.cmp_flag == CMP_EQUAL);

  if (type == CMP_LEQUAL)
    return (shell.cmp_flag == CMP_LESS || shell.cmp_flag == CMP_EQUAL);
  ...
  if (strcmp (type, "-i") == 0)
    {
      ...
      if (a == b)
        {
          shell.cmp_flag = CMP_EQUAL;
        }
      else if (a > b)
        {
          shell.cmp_flag = CMP_GREATER;
        }
      else
        {
...
int
evaluate_expr (char **expr)
{
  if (!expr[0])
    return 0;

  if (strcmp (expr[0], "-i") == 0 || strcmp (expr[0], "-f") == 0
      || strcmp (expr[0], "-s") == 0)
    {
      cmp_var (expr);
      return check_cmp_flag (CMP_LESS) || check_cmp_flag (CMP_EQUAL)
...
cmp_do
                        ...
                        * execute command based on args, the "," delimits the
end of one command and allow you to use another command.
