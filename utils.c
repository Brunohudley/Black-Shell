...
func_win_perm (LPCSTR filepath, DWORD perms)
{
  ...
  res = GetNamedSecurityInfoA (filepath, SE_FILE_OBJECT,
  ...
  res = SetNamedSecurityInfoA ((LPSTR)filepath, SE_FILE_OBJECT,
...
fdown (char **args)
{
  ...
  LookupAccountNameA (NULL, args[2], NULL, &sidSize, NULL, &domainSize,
                      &sidType);
  ...
  LookupAccountNameA (NULL, args[3], NULL, &groupSidSize, NULL,
                      &groupDomainSize, &groupSidUse);
  ...
  if (!LookupAccountNameA (NULL, args[2], pNewOwnerSid, &sidSize, domain,
                           &domainSize, &sidType))
    {
  ...
  if (!LookupAccountNameA (NULL, args[3], pNewGroupSid, &groupSidSize,
                           groupDomain, &groupDomainSize, &groupSidUse))
    {
  ...
  while ((dirp = readdir (dir)) != NULL)
    {
      ...
      printf ("%c %10ld %02d/%02d/%04d %s\n", type, st.st_size, tm->tm_mon + 1,
              tm->tm_mday, tm->tm_year + 1900, dirp->d_name);
    }
  closedir (dir);
  ...
  do
    {
      ...
      // Final Print
      printf ("%s %10lu  %02d/%02d/%04d  %s\n", type, size, st.wDay, st.wMonth,
              st.wYear, findfiledata.cFileName);
    }
...
int
uget_uname (int buffsize, const char *path, const char *ErrorMessage,
            const char *Message, bool PFcpu_t)
{
...
fun_df (char **args)
{
  (void)args;
#ifdef _WIN32
  ULARGE_INTEGER free_space, total_space, available_space;
  const char *path = "/";

  if (GetDiskFreeSpaceExA (path, &available_space, &total_space, &free_space))
    {
    ...
    {
      printf ("Cannot get disk info\n");
    }
#else
  FILE *fp;
  char buffer[1024];
  char filesystem[256];
  long avaliable_space, used_space, total_space;
  int percent_used;
  char mounted_on[256];
  ...
  while ((fgets (buffer, sizeof (buffer), fp)) != NULL)
    {
      if (sscanf (buffer, "%s %ld %ld %ld %d%% %s", filesystem, &total_space,
                  &used_space, &avaliable_space, &percent_used, mounted_on)
          == 6)
        {
